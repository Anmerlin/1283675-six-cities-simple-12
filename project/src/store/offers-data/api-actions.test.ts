...
describe('Async actions', () => {
  ...
  it('should fetchTargetOfferAction when server returns 200', async () => {
    ...
    const reviewsIncoming = makeFakeReviews();
    ...
    mockAPI.onGet(`${APIRoute.Comments}/${hotelId}`).reply(200, reviewsIncoming);
  ...
  it('should dispatch sendReviewAction when server returns 200', async () => {
    ...
    const reviewOutgoing = makeFakeReviewData();
    const reviewsIncoming = makeFakeReviews();

    const store = mockStore();
    mockAPI.onPost(`${APIRoute.Comments}/${targetId}`).reply(200, reviewsIncoming);
